...
**最新更新！** 本仓库现已集成 Coin3D 源码作为子模块，可以与主项目一起编译：

✅ **统一构建** - 一次编译命令构建 Coin3D 和所有示例  
✅ **源码调试** - 在 IDE 中直接调试 Coin3D 源码  
✅ **输出管理** - 所有 DLL、PDB 统一输出到 `bin/` 目录  
✅ **版本控制** - 通过 Git 子模块管理 Coin3D 版本  

📖 **详细说明**：查看 [Coin3D 集成构建文档](docs/COIN3D_INTEGRATION.md)
...
coin3d_selfdemo/
├── thirdparty/
│   └── coin/              # Coin3D 源码（Git 子模块）⭐ NEW
├── coin3d_examples/
│   ├── basic_shapes/      # 基本形状示例
│   ├── scene_graph/       # 场景图操作示例
│   ├── transformations/   # 变换示例
│   ├── materials/         # 材质和外观示例
│   ├── lighting/          # 光源示例
│   ├── cameras/           # 相机操作示例
│   ├── text/              # 文本渲染示例
...
│   ├── file_io/           # 文件读写示例
│   ├── animation/         # 动画示例
│   ├── rendering_pipeline/  # 渲染流程深入分析示例
│   └── gpu_data_transfer/   # GPU数据传输优化示例
├── docs/                  # 文档
└── bin/                   # 编译输出目录（DLL、EXE、PDB）⭐ NEW
```
...
Coin3D 已作为子模块集成，无需单独安装！只需要：

- **CMake** (>= 3.15)
- **SoQt** - Coin3D 的 Qt 绑定
- **Qt** (Qt5 或 Qt6)
- **C++ 编译器** (支持 C++11 用于 Coin3D，C++17 用于示例)
- **OpenGL** 开发库
...
sudo apt-get install libsoqt-dev  # 只需要 SoQt，Coin3D 从源码构建
...
```bash
brew install cmake
brew install soqt  # 只需要 SoQt
brew install qt
```

### 使用外部 Coin3D（可选）

如果你仍然想使用系统安装的 Coin3D：
```
